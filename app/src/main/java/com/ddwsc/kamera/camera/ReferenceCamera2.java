package com.ddwsc.kamera.camera;
...
public class ReferenceCamera2 extends ReferenceCamera {
    ...
    private void initializeCameraIds() {
        try {
            // nhận thông tin về camera trong hệ thống
            CameraManager manager = (CameraManager) mContext.getSystemService(Context.CAMERA_SERVICE);
            // lấy một danh sách các đặc tính cho mỗi máy ảnh
            for (String cameraId : manager.getCameraIdList()) {
                // CameraCharacteristics chứa một số lượng key rất lớn có thể sử dụng để chứa thông tin về camera
                CameraCharacteristics characteristics
                        = manager.getCameraCharacteristics(cameraId);
                int facing = characteristics.get(CameraCharacteristics.LENS_FACING); // lấy hướng camera
                if (facing == CAMERA_FACING_FRONT) {
                    mFrontCameraIds.add(cameraId); // camera trước
                }else if (facing == CAMERA_FACING_BACK) {
                    mRearCameraIds.add(cameraId); // camera sau
                }
            }
